...
async function handleMultiUpload(req, res) {
  try {
    ...
    // FIX: Accept singular keys (sent by frontend for batch uploads) as fallbacks
    const {
      titles,
      ...
      is_free,
      approval_status,
      states, state // Frontend might send 'state'
    } = req.body;

    // Helper to ensure array, but also handle singular fallback if array is missing
    const getList = (plural, singular) => {
      if (plural) return Array.isArray(plural) ? plural : [plural];
      if (singular) return [singular]; // Treat singular as a single-item list, or we'll handle index lookup carefully
      return [];
    };

    const titleList = getList(titles, null);

    // For metadata that applies to ALL files if singular is provided:
    const globalMatType = material_type;
    const typeList = getList(material_types, material_type);

    const globalField = field;
    const fieldList = getList(fields, field);

    const globalCourse = course;
    const courseList = getList(courses, course);

    const globalSubject = subject;
    const subjectList = getList(subjects, subject);

    const globalUni = university_name;
    const uniList = getList(university_names, university_name);

    const globalState = state;
    const stateList = getList(states, state);

    // is_free comes as an array of strings "true"/"false" or booleans
    const isFreeList = getList(is_free, null);
    ...
    for (let i = 0; i < req.files.length; i++) {
      ...
      try {
        ...
        for (let i = 0; i < req.files.length; i++) {
          const f = req.files[i];
          const title = (titleList.length === req.files.length ? titleList[i] : (titleList[0] || f.originalname));

          // Use index if list matches file count, otherwise use first element (global), fallback to default
          const getValue = (list, def) => {
            if (!list || list.length === 0) return def;
            // If list length matches file count, use index. Note: This assumes aligned arrays.
            // If list length is 1, assume global.
            if (list.length === req.files.length) return list[i];
            if (list.length === 1) return list[0];
            return list[i] || def;
          };

          const matType = getValue(typeList, 'personal_material');
          const isFreeVal = getValue(isFreeList, true) === 'true' || getValue(isFreeList, true) === true;
          ...
          try {
            ...
            const currentField = getValue(fieldList, null);
            const currentCourse = getValue(courseList, null);
            const currentSubject = getValue(subjectList, null);
            const currentUni = getValue(uniList, null);
            const currentState = getValue(stateList, null);
          ...
          `;
              insertVals = [
                ...
                'university_material', currentUni, currentCourse, currentSubject,
                isFreeVal, startStatus, currentState
              ];
            } else {
              // personal
              const institutionType = ["Class 12", "Class 11", "Class 10"].includes(currentField) ? "School" : "College";
              insertSql = `
            INSERT INTO notes (
              user_id, title, pdf_path, file_url, cloudinary_public_id,
          ...
          `;
              insertVals = [
                userId, title, pdfPath || null, fileUrl || null, publicId || null,
                'personal_material', institutionType, currentField, currentCourse, currentSubject,
    ...
    async function serveNoteWithWatermark(req, res) {
      try {
        ...
        console.log(`[DEBUG] serveNoteWithWatermark called for Note ID: ${id} by User: ${viewingUser.username} (${viewingUser.id})`);

        const note = await findNoteByIdAndJoinUser(id);
        if (!note) {
          console.log(`[DEBUG] Note not found: ${id}`);
          return res.status(404).json({ error: "Note not found." });
        }

        console.log(`[DEBUG] Note found. Title: ${note.title}, Type: ${note.material_type}, Owner: ${note.user_id}, URL: ${note.file_url}`);
        ...
        if (note.material_type !== 'university_material') {
          ...
          if (viewingUser.role !== 'admin' && String(note.user_id) !== String(viewingUser.id) && note.owner_role !== 'admin') {
            ...
            if (permResult.rows.length === 0) {
              console.log(`[DEBUG] Access denied for User ${viewingUser.id} on Personal Note ${id}`);
        ...
        if (note.cloudinary_public_id || note.file_url) {
          console.log(`[DEBUG] Fetching from Cloudinary: ${note.file_url}`);
          ...
          if (!resp.ok) {
            console.error(`[DEBUG] Failed to fetch remote PDF. Status: ${resp.status} ${resp.statusText}`);
            throw new Error(`Failed to fetch remote PDF: ${resp.statusText}`);
          }

          console.log(`[DEBUG] Remote PDF fetched. Buffering...`);
          const remoteBuffer = Buffer.from(await resp.arrayBuffer());

          console.log(`[DEBUG] Loading PDFDocument...`);
          ...
          if (viewingUser.role !== 'admin' && String(note.user_id) !== String(viewingUser.id)) {
            console.log(`[DEBUG] Watermarking PDF...`);
        ...
        console.log(`[DEBUG] Serving local file: ${notePath}`);
        ...
        console.error("❌ Error serving PDF:", err); // Log full error object
        if (err && err.code === 'ENOENT') {
          return res.status(500).json({ error: "Could not serve the note. PDF file or logo asset is missing on the server." });
        }
        res.status(500).json({ error: "Could not serve the note. Details in server logs." });
      }
    }
