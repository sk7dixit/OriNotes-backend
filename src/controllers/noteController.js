...
console.log('TRACE: Imports done');
...
async function handleMultiUpload(req, res) {
  try {
    ...
    // Helper to pick value for index i
    const getValue = (list, i, def) => {
    ...
    for (let i = 0; i < req.files.length; i++) {
      const f = req.files[i];
      try {
        const currentTitle = (titleList.length === req.files.length ? titleList[i] : (titleList[0] || f.originalname));
        const matType = getValue(typeList, i, 'personal_material');
        const isFreeVal = getValue(isFreeList, i, true) === 'true' || getValue(isFreeList, i, true) === true;
        const currentField = getValue(fieldList, i, null);
        const currentCourse = getValue(courseList, i, null);
        const currentSubject = getValue(subjectList, i, null);
        const currentUni = getValue(uniList, i, null);
        const currentState = getValue(stateList, i, null);
        ...
        if (process.env.CLOUDINARY_API_KEY && process.env.CLOUDINARY_API_SECRET) {
          try {
            ...
            console.warn("Cloudinary upload failed for file " + f.originalname + ", falling back to local:", cloudErr.message);
          }
        }
        ...
        if (matType === 'university_material') {
          insertSql = `
              ...
              `;
          insertVals = [
            userId, currentTitle, pdfPath || null, fileUrl || null, publicId || null,
        ...
        console.error('Upload failed for file', f.originalname, innerErr);
        errors.push({ file: f.originalname, error: innerErr.message || String(innerErr) });
      }
    }
...
console.log('TRACE: Reaching exports');
module.exports = {
  uploadUserNote,
  handleMultiUpload,
  getFilteredNotes,
