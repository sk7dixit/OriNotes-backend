...
// --- ADMIN/SYSTEM: Create and send a new notification ---
async function createNotification(req, res) {
  const { title, message, recipient_id, type = 'general', reference_id, reference_url } = req.body;
  ...
  try {
    const result = await pool.query(
      ...
      [title, message, recipient_id || null, type, reference_id, reference_url]
    );
...
// --- USER: Get all their notifications (Global + Targeted) ---
async function getUserNotifications(req, res) {
  const userId = req.user.id;
  try {
    const result = await pool.query(`
      SELECT
        n.id, n.title, n.message, n.created_at, n.type, n.reference_url,
        un.is_read
